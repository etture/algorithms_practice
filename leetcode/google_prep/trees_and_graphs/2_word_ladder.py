...
class Solution:
    def ladderLength_wrong(self, beginWord: str, endWord: str, wordList: List[str]) -> int:
        def create_transform_table(dictionary: List[str]):
            ...
            trf_tbl = dict()
            # trf_tbl = defaultdict(lambda: set())
            for idx, word in enumerate(dictionary):
                # print(len(trf_tbl))
                if word not in trf_tbl:
                    trf_tbl[word] = list()
                    # print(f'     {word} not in trf_tbl')
                if idx < len(dictionary) - 1:
                    for i in range(idx + 1, len(dictionary)):
                        other_word = dictionary[i]
                        if check_one_char_diff(word, other_word):
                                
                            if word not in trf_tbl:
                                trf_tbl[word] = list()
                            if other_word not in trf_tbl:
                                trf_tbl[other_word] = list()
                            if other_word not in trf_tbl[word]:
                                trf_tbl[word].append(other_word)
                            if word not in trf_tbl[other_word]:
                                trf_tbl[other_word].append(word)
                            
                            # if word == 'ph' or other_word == 'ph':
                            #     # print(f'w1: {word}, w2: {other_word}')
                            #     pprint(trf_tbl['ph'])
                            #     pprint(trf_tbl['pa'])
            return trf_tbl
        

        all_dict = [beginWord] + wordList
        transform_table = create_transform_table(all_dict)
        # print(f'transform_table ready')
        # pprint(transform_table)

        path_to_target = dict()

        # def find_shortest_path(cur_word, target_word, history: Set[str]) -> int:
        def find_shortest_path(cur_word, target_word, history: List[str]):
            candidates = transform_table[cur_word]
            # print(f'cur_word: {cur_word}, candidates: {candidates}, history: {history}')
            if cur_word in path_to_target:
                # return len(history) + path_to_target[cur_word][0], history + path_to_target[1]
                return path_to_target[cur_word]
            elif len(candidates) == 0:
                path_to_target[cur_word] = (0, [cur_word] + ['X'])
                return 0, [cur_word] + ['X']
            elif len(history) >= len(all_dict):
                # path_to_target[cur_word] = (0, history + ['X'])
                return 0, [cur_word] + ['X']
            elif target_word in candidates:
                # if cur_word not in path_to_target or (cur_word in path_to_target and path_to_target[cur_word][0] > len(history) + 1):
                #     path_to_target[cur_word] = (len(history) + 1, history + [target_word])
                #     return len(history) + 1, history + [target_word]
                # else:
                #     return history + path_to_target[cur_word]
                path_to_target[cur_word] = (2, [cur_word, target_word])
                return 2, [cur_word, target_word]
            else:
                candidate_paths = list()
                for next_cur in candidates:
                    if next_cur not in history and next_cur != beginWord:
                        shortest_path = find_shortest_path(
                                next_cur,
                                target_word,
                                # history | set([next_cur])
                                history + [next_cur]
                            )
                        candidate_paths.append(shortest_path)
                        # if cur_word in ['cm', 'tm', 'sm']: 
                            # print(f'cur_word: {cur_word}, next_cur: {next_cur}, fsp: {shortest_path}')
                valids = [path for path in candidate_paths if path[0] > 0]
                # if cur_word in ['cm', 'tm', 'sm']: 
                    # print(f'    "{cur_word}" -> history: {history}, candidates: {candidates}, valids: {valids}')
                if len(valids) > 0:
                    min_path = tuple()
                    min_val = 9999999
                    for v in valids:
                        if v[0] < min_val:
                            min_val = v[0]
                            min_path = v
                    # path_to_target[cur_word] = min([v[0] for v in valids])
                    path_to_target[cur_word] = (min_path[0] + 1, [cur_word] + min_path[1])
                    # return min(valids)
                    return path_to_target[cur_word]
                path_to_target[cur_word] = (0, history + ['X'])
                return 0, history + ['X']
        ...
        # print(f'beginword: {beginWord}, candidates')
        # return find_shortest_path(beginWord, endWord, set([beginWord]))
        shortest = find_shortest_path(beginWord, endWord, [beginWord])
        print(f'shortest: {shortest}')
        # return find_shortest_path(beginWord, endWord, [beginWord])[0]
        # pprint(path_to_target)
        return shortest[0]
