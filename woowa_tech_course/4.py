...
def solution(pobi, crong):
    ...
    if pobi[right] - pobi[left] != 1 or crong[right] - crong[left] != 1:
        return -1
    else:
        pobi_max = get_max(pobi)
        crong_max = get_max(crong)
...
def get_max(pair):
    left_str = [num for num in str(pair[left])]
    right_str = [num for num in str(pair[right])]
    ...
    for num in left_str:
        pot[left_add] += int(num)
        pot[left_multiply] *= int(num)

    for num in right_str:
        pot[right_add] += int(num)
        pot[right_multiply] *= int(num)
