...
def solution(totalTicket, logs):
    answer = []
    remaining_ticket = totalTicket
    success_id_set = set()
    logs_split = [log.split(' ') for log in logs]
    time_split = [list(map(int, log[time].split(':'))) for log in logs_split]

    ticketing_ongoing = False
    current_ticketing_user = ''
    ongoing_ticketing_start_time = []
    for index, log in enumerate(logs_split):
        if remaining_ticket < 1:
            break

        if not (ticketing_ongoing) and current_ticketing_user == '' and log[action] == 'request':
            current_ticketing_user = log[userid]
            ticketing_ongoing = True
            ongoing_ticketing_start_time = time_split[index]
        elif ticketing_ongoing:
            if one_minute_passed(ongoing_ticketing_start_time, time_split[index]):
                success_id_set.add(current_ticketing_user)
                remaining_ticket -= 1
                ticketing_ongoing = False
                current_ticketing_user = ''
                ongoing_ticketing_start_time = []
            elif log[userid] == current_ticketing_user and log[action] == 'leave':
                ticketing_ongoing = False
                current_ticketing_user = ''
                ongoing_ticketing_start_time = []

    if current_ticketing_user != '':
        success_id_set.add(current_ticketing_user)
    answer = list(success_id_set)
    answer.sort()

    return answer


def one_minute_passed(start_time, end_time):
